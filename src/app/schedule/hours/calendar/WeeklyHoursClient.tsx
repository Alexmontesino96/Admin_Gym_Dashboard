...
export default function WeeklyHoursClient() {
  ...
  const getWeekLabel = (weekStart: Date): string => {
    // Usar siempre las fechas calculadas para mostrar el rango solicitado
    const endDate = addDays(weekStart, 6)
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {daysOfWeek.map((dayInfo, index) => {
          // Calcular la fecha esperada para este día de la semana
          const expectedDate = addDays(currentWeekStart, index)
          const expectedDateStr = formatDate(expectedDate)
          
          // Buscar los datos de la API para esta fecha específica
          const dayData = weeklyHours.find(h => h.date === expectedDateStr)
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {expectedDate.toLocaleDateString('es-ES', { 
                        day: '2-digit', 
                        month: '2-digit' 
                      })}
                    </p>
                  ...
                  {dayData ? (
                    <>
                      <div className="flex justify-center">
                        {/* Determinar si es realmente especial o solo aplicado desde plantilla */}
                        {dayData.is_special && dayData.description && !dayData.description.includes('Aplicado desde plantilla') ? (
                          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800">
                            Especial
                          </span>
                        ) : dayData.is_closed ? (
                      ...
                      {/* Solo mostrar descripción si es realmente especial (no aplicado desde plantilla) */}
                      {dayData.is_special && dayData.description && !dayData.description.includes('Aplicado desde plantilla') && (
