...
export default function ScheduleClient() {
  ...
  const buildDetailJSX = (detail:any): React.ReactElement => {
    ...
    const start = new Date(ses.time_info?.local_time || ses.start_time_local || ses.start_time)
          ...
          {loadingSessions ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSessions.map(item=>{
                ...
                const start = new Date(s.time_info?.local_time || s.start_time_local || s.start_time)
      ...
      {showDeleteSessionModal && deleteSessionTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-700 mb-6">¿Seguro que deseas eliminar la sesión del <span className="font-semibold">{new Date(deleteSessionTarget.time_info?.local_time || deleteSessionTarget.start_time_local || deleteSessionTarget.start_time).toLocaleString('es-ES',{weekday:'long', day:'numeric', month:'short', hour:'2-digit', minute:'2-digit'})}</span>?</p>
      ...
      {showCancelSessionModal && cancelSessionTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <p className="text-sm text-gray-700 mb-6">¿Seguro que deseas cancelar la sesión del <span className='font-semibold'>{new Date(cancelSessionTarget.time_info?.local_time || cancelSessionTarget.start_time_local || cancelSessionTarget.start_time).toLocaleString('es-ES',{weekday:'long',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}</span>?</p>
            <div className="flex justify-end space-x-3 text-sm">
